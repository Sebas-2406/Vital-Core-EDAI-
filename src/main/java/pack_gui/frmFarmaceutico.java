...
public class frmFarmaceutico extends javax.swing.JFrame {
    ...
    private void cargarTablas() {
        ...
        // --- INICIO DE IMPLEMENTACIÓN DE COLA (FIFO) ---
        pack_estructuras.Cola<String[]> colaPedidos = new pack_estructuras.Cola<>();

        // 1. Cargar Recetas Pendientes del médico en la Cola
        try (java.io.BufferedReader br = new java.io.BufferedReader(new java.io.FileReader("recetas.csv"))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (datos.length >= 5 && datos[4].equalsIgnoreCase("Pendiente")) {
                    colaPedidos.encolar(new String[]{datos[0], datos[1], datos[2], datos[3], datos[4]});
                }
            }
        } catch (Exception e) { /* El archivo puede no existir aún */ }
        
        // 2. Cargar Pedidos de Medicamentos de Pacientes en la misma Cola
        try (java.io.BufferedReader br = new java.io.BufferedReader(new java.io.FileReader("pedidos_medicamentos.csv"))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (datos.length >= 6 && datos[5].equalsIgnoreCase("Pendiente")) {
                    colaPedidos.encolar(new String[]{
                        "Pedido-" + datos[0], // ID
        ...
        // 3. Vaciar la Cola en la tabla visual (El primero que llegó será el primero en la tabla)
        while (!colaPedidos.estaVacia()) {
            modRecetas.addRow(colaPedidos.desencolar());
        }
        // --- FIN DE IMPLEMENTACIÓN DE COLA ---

        // Cargar inventario de medicinas normalmente
    ...
    private void actualizarTablaInventario() {
        ...
        try (java.io.BufferedReader br = new java.io.BufferedReader(new java.io.FileReader(archivo))) {
            ...
            while ((linea = br.readLine()) != null) {
                ...
                String[] datos = linea.split(",");
                if (datos.length == 2) {
                    filasInv.agregar(datos);
                }
            }

            modelo.setRowCount(0);
            for (int i = 0; i < filasInv.getTamaño(); i++) {
                modelo.addRow(filasInv.obtener(i));
            }
        } catch (java.io.IOException e) {
    ...
    private void bDespacharMedicinaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bDespacharMedicinaActionPerformed
            ...
            while ((linea = br.readLine()) != null) {
                ...
                if (datos.length == 2) {
                    ...
                    if (med.equalsIgnoreCase(nombreMedicina)) {
                        stock += cantidadNueva; // Aumentar stock
                        medicinaEncontrada = true;
                    }
                    inventario.agregar(new String[]{med, String.valueOf(stock)});
                }
            }
        } catch (java.io.IOException e) {}
        ...
        if (!medicinaEncontrada) {
            ...
            inventario.agregar(new String[]{nombreNormalizado, String.valueOf(cantidadNueva)});
        }

        // Escritura en el archivo usando la Lista Enlazada
        try (java.io.PrintWriter pw = new java.io.PrintWriter(new java.io.FileWriter(archivo))) {
            pw.println("MEDICINA,STOCK");
            for (int i = 0; i < inventario.getTamaño(); i++) {
                String[] med = inventario.obtener(i);
                pw.println(med[0] + "," + med[1]);
            }
        } catch (java.io.IOException e) {
